...
function initSocket(username) {
  ...
  _socket.on('dm:message', (msg) => {
    ...
    if (activeChannelId === channelId) {
      ...
      showToast(`Message de ${msg.authorName}: ${msg.content.slice(0, 40)}${msg.content.length > 40 ? '…' : ''}`);
    }
  });
...
function socketSendDm(content) {
  ...
  _socket.emit('dm:send', { toId: _currentRealDmPartnerId, content: content.trim() });
}
...
function selectRealDmChannel(friendId, friendName) {
  ...
  document.querySelectorAll('.channel-item, .dm-item, .real-friend-item').forEach(el => {
  ...
  if (SESSION_MESSAGES[channelId]) {
    ...
    container.innerHTML = '<div class="real-dm-loading">Chargement des messages…</div>';
...
function updateRealFriendStatus(userId, status) {
  const el = document.querySelector(`.real-friend-item[data-friend-id="${userId}"] .dm-status`);
  if (el) {
...
function updateRealDmBadge(convId, count) {
  const unreadCount = count !== undefined ? count : (_realUnread.has(convId) ? 1 : 0);
  const el = document.querySelector(`.real-friend-item[data-conv-id="${convId}"] .unread-badge`);
  if (el) {
    el.style.display = unreadCount > 0 ? '' : 'none';
  }
  // Ajoute/retire la classe has-unread
  const item = document.querySelector(`.real-friend-item[data-conv-id="${convId}"]`);
